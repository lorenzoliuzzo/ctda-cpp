...
namespace ctda {
    ...
    namespace math {
        ...
        template <typename T1, typename T2>
            requires (are_same_quantity_v<T1, T2>)
        struct add_impl<T1, T2> {

            using result_t = quantity<add_t<typename T1::value_t, typename T2::value_t>, typename T1::unit_t>;
        ...
        template <typename T1, typename T2>
            requires (is_quantity_v<T1> && std::is_same_v<typename T1::base_t, dimensionless>)
        ...
        template <typename T1, typename T2>
            requires (std::is_arithmetic_v<T2> || is_complex_v<T2> || std::is_array_v<T2>)
                && (is_quantity_v<T2> && std::is_same_v<typename T2::base_t, dimensionless>)
        struct add_impl<T1, T2> {
        ...
        /// @brief Add specialization for measurements
        template <typename T1, typename T2>
            requires (are_same_quantity_v<T1, T2>)
        struct add_impl<measurement<T1>, measurement<T2>> {

            using result_t = measurement<quantity<add_t<typename T1::value_t, typename T2::value_t>, typename T1::unit_t>>;

            static constexpr result_t f(const measurement<T1>& x, const measurement<T2>& y) noexcept {
                
                return { x.val + y.val, math::sqrt(math::sq(x.unc) + math::sq(y.unc)) };

            }

        };

        /// @brief Add specialization for measurements and numbers, complex numbers, arrays
        template <typename T1, typename T2>
            requires (is_quantity_v<T1> && std::is_same_v<typename T1::base_t, dimensionless>)
                && (std::is_arithmetic_v<T2> || is_complex_v<T2> || std::is_array_v<T2>)
        struct add_impl<measurement<T1>, T2> {

            using result_t = measurement<quantity<add_t<typename T1::value_t, T2>, typename T1::unit_t>>;

            static constexpr result_t f(const T1& x, const T2& y) noexcept {
                return { x.val + y, x.unc };
            }
        ...
        template <typename T1, typename T2>
            requires (std::is_arithmetic_v<T2> || is_complex_v<T2> || std::is_array_v<T2>)
                && (is_quantity_v<T2> && std::is_same_v<typename T2::base_t, dimensionless>)
        struct add_impl<T1, measurement<T2>> {

            using result_t = measurement<quantity<add_t<T1, typename T2::value_t>, typename T2::unit_t>>;

            static constexpr result_t f(const T1& x, const T2& y) noexcept {
                return { x + y.val, y.unc };
            }
